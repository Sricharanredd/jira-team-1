...
def create_project(
    ...
    try:
        ...
        # CRITICAL: Assign Creator as ADMIN explicitly
        print(f"DEBUG: assigning creator {current_user.id} as ADMIN for project {new_project.id}")
        auth_crud.assign_role(db, current_user.id, new_project.id, auth_models.RoleType.ADMIN)
        
        return new_project
    except Exception as e:
        print(f"ERROR: create_project failed: {e}")
...
def get_project(
    ...
    if current_user.global_role != auth_models.GlobalRole.ADMIN and not role:
        raise HTTPException(status_code=403, detail="Access denied. Not a member.")
...
def delete_project(
    ...
    # TODO: Implement actual delete logic in CRUD if needed
    # db.delete(project)
    # db.commit()
    return {"message": "Project deleted (simulation)"}
    ...
    for m in members:
        ...
        if user:
            result.append({
                ...
                "name": user.name,
                "email": user.email,
                "role": m.role.value if hasattr(m.role, 'value') else m.role # Normalize
            })
    return result
...
def get_project_assignees(
    ...
    for m in members:
        ...
        if user:
            result.append({
                "value": user.name, # For the dropdown value
                "label": f"{user.name} ({m.role.value if hasattr(m.role, 'value') else m.role})", # Label
                "id": user.id,
