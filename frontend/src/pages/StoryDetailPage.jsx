...
const StoryDetailPage = () => {
    ...
    // Context Data
    const { userRole, issues } = useProject(); // Get role and issues for sprints
    ...
    // Remove local userRole state since we use context now
    ...
    // Fetch Members and Sprints
    useEffect(() => {
        if (story?.project_id) {
            // Fetch Members
            api.get(`/projects/${story.project_id}/members`)
    ...
    useEffect(() => {
        if (issues.length > 0) {
            const sprints = [...new Set(issues.map(i => i.sprint_number).filter(Boolean))].sort((a,b) => a - b);
    ...
    const fetchData = async () => {
        try {
            const [storyRes, historyRes] = await Promise.all([
            ...
            // Manual role calculation removed - using Context
            ...
            setFormData({
                ...
                status: storyRes.data.status || 'todo',
            ...
            <div className="px-8 py-6 pb-0 bg-white">
                ...
                <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2 text-xs text-[#5E6C84]">
                         <div className="flex items-center gap-1 cursor-pointer hover:text-blue-700">
                            ...
                            <span className="hover:underline" onClick={() => {
                                 ...
                                 if (context === 'backlog') navigate(`/projects/${story.project_id}/backlog`);
                                 else navigate(`/projects/${story.project_id}`);
                            }}>
                                {story.project_name}
                            </span>
                         </div>
                    ...
                    <button 
                        onClick={() => {
                             const context = searchParams.get('context');
                             if (context === 'backlog') navigate(`/projects/${story.project_id}/backlog`);
                             else if (context === 'issues') navigate(`/projects/${story.project_id}/issues`);
                             else if (context === 'timeline') navigate(`/projects/${story.project_id}/timeline`);
                             else if (context === 'calendar') navigate(`/projects/${story.project_id}/calendar`);
                             else if (context === 'assigned_to_me') navigate(`/assigned-to-me`);
                             else navigate(`/projects/${story.project_id}`); // Default to board
                        }}
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-2">
                        {/* EDIT Button: Only Admin, SM, Developer */}
                        {['ADMIN', 'SCRUM_MASTER', 'DEVELOPER'].includes(userRole) && (
                            <button 
                        ...
                        <button className="flex items-center gap-1.5 hover:bg-[#EBECF0] text-[#42526E] px-3 py-1.5 rounded-[3px] font-medium text-sm transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                            </svg>
                            Comment
                        </button>
                        <button className="hover:bg-[#EBECF0] text-[#42526E] px-3 py-1.5 rounded-[3px] font-medium text-sm transition-colors">
                            Assign
                        </button>
                        <button className="hover:bg-[#EBECF0] text-[#42526E] px-3 py-1.5 rounded-[3px] font-medium text-sm transition-colors flex items-center gap-1">
                            More
                    ...
                    <div className="flex items-center gap-1">
                        {['todo', 'in_progress', 'done'].map((status) => (
                             <button
                                key={status}
                                className={`px-3 py-1.5 text-xs font-semibold uppercase border rounded-[3px] transition-colors ${
                                    story.status === status 
                                     ...
                                     <div className="space-y-3">
                                         <div className="flex">
                                             ...
                                             <div className="flex items-center gap-1.5">
                                                 <span className={`w-3.5 h-3.5 rounded-[2px] flex items-center justify-center ${
                                                      story.issue_type === 'bug' ? 'bg-[#ff5630]' :
                                         ...
                                         <div className="flex">
                                             <span className="w-32 text-sm text-[#5E6C84] flex-shrink-0">Sprint:</span>
                                             <span className="text-sm text-[#172B4D]">{story.sprint_number ? `Sprint ${story.sprint_number}` : 'None'}</span>
                                         </div>
                                         <div className="flex">
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    {/* Permission Check: Only ADMIN or SCRUM_MASTER can edit Assignee */}
                                    {['ADMIN', 'SCRUM_MASTER'].includes(userRole) ? (
                                        <select
                                            value={formData.assignee}
                                            onChange={(e) => setFormData({...formData, assignee: e.target.value})}
                                            className="w-full border border-[#DFE1E6] hover:border-[#B3BAC5] rounded-[3px] px-2 py-1.5 text-sm cursor-pointer"
                                        >
                                        ...
                                        <div className="w-full bg-gray-100 border border-gray-200 rounded-[3px] px-2 py-1.5 text-sm text-gray-500 cursor-not-allowed">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-[#5E6C84] uppercase mb-1.5">Reviewer</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className={`w-full border border-[#DFE1E6] hover:border-[#B3BAC5] rounded-[3px] px-2 py-1.5 text-sm cursor-pointer ${formData.status === 'backlog' ? 'bg-gray-100 text-gray-400 cursor-not-allowed' : ''}`}
                                        disabled={formData.status === 'backlog'}
                                    >
                                        <option value="">{formData.status === 'backlog' ? 'Backlog (No Sprint)' : 'No Sprint'}</option>
